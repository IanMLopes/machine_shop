  ...
  <b-navbar id="template-header" class="navbar default-layout col-lg-12 col-12 p-0 fixed-top d-flex flex-row" toggleable="md">
    <div class="text-center navbar-brand-wrapper d-flex align-items-top justify-content-center">
      <router-link class="navbar-brand brand-logo" to="/">
        <!-- <img src="../../assets/images/LOGO-cort.png" alt="logo" style="width: 100px; height: 50px"  /> -->
        <span style=" font-weight: normal; margin-top:10px ">ICCT</span>
      </router-link>
          ...
          <template slot="button-content">
            <div class="d-flex align-items-center">
              ...
              <!--:src="'data:image/jpeg;base64,'+imageProfile" -->
                  <!--img-xs style="width: 80px; height: 80px" -->
            </div>
          </template>
          <b-dropdown-item>
            <div class="dropdown-header text-center w-100">
              <img class="img-md rounded-circle" :src="imageProfile" alt="Profile image">
              <p class="mb-1 mt-3 font-weight-semibold">{{dataUser.name_user}}</p>
              <p class="font-weight-light text-muted mb-0">{{dataUser.email_user}}</p>
            </div>
          </b-dropdown-item>
          <b-dropdown-item   @click="$router.push('/profile-user')"> Meus Dados <!-- <span class="badge badge-pill badge-danger"> --> </span> <i class="dropdown-item-icon ti-dashboard"></i> </b-dropdown-item>
          <!--<b-dropdown-item  >Messages<i class="dropdown-item-icon ti-comment-alt"></i></b-dropdown-item> -->
          <b-dropdown-item @click="logout()" >Sair<i class="dropdown-item-icon ti-power-off"></i></b-dropdown-item>
        </b-nav-item-dropdown>
      </b-navbar-nav>
...
export default {
  ...
  mounted(){
     bus.$on('decoded', data => {
      ...
      // this.imageProfile = data.path_photo;
      // console.log("this.imageProfile", this.imageProfile)
      // this.dataUser = data;
     });

     this.imageProfile = 'data:image/jpeg;base64,'+this.dataUser.path_photo;
