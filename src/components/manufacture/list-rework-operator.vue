          ...
          <div class='card-body'>
            <div class='col-lg-12 col-md-12 mt-md-0 mt-4'>
                 <div>
                    ...
                    <p class="mb-5 mr-auto">Retrabalho para executar</p>
                  </div>
              <div class='d-flex'>
                            ...
                            <div class="modal-content">
                                    ...
                                    <table class="table">
                                          <thead class="table-success">
                                            <tr class="text-center border">
                                              <th class="border p-0" style="vertical-align: middle; width: 20%"><p style="margin: auto">Retrabalho</p></th>
                                    ...
                                    <table  class="table table-hover" style="white-space: unset; border-spacing: 0;">
                                      <thead  class="accordion-item">
                                        <tr class="text-center border">
                                            ...
                                            <td class="border p-0" style="width: 20%">{{taskDetails.name_task}}</td>
                                            ...
                                            <td v-else class="border p-0" style="width: 30%; color: orange; font-size:32px">
                                              <div class="d-flex justify-content-center">
                                                <span class="p-0 m-0 mr-2">{{day == 0 ? '00' : day+' - '}}  </span>
                              ...
                              <div class="modal-footer  col-lg-12 d-flex p-4 justify-content-start">
                                <div class="d-flex col-lg-12" >
                                  <button :disabled="taskDetails.status_task == 2 || taskDetails.status_task == 3 && iniciarNone == false || taskDetails.status_task == 4" 
...
;     } else if(val == 2 ) {
    ...
    async showModalInfoDrawing(task){
      ...
      console.log(" taskDetails selecionada --> ", this.taskDetails)

      if(this.taskDetails.status_task == 2){
         let id_rework = this.taskDetails.id_rework;
        this.continueTask(id_rework);
      }
    ...
    startTask(){   
      console.log("detalhes rework ==>", this.taskDetails)

          this.isLoading = true;
          let dataInitialTask = moment(new Date()).format("DD/MM/YYYY HH:mm:ss");      
          let json = {
                      id_rework: this.taskDetails.id_rework,
          ...
          this.axios.post('/rework/start', json)
          .then((response) => {
            console.log("response start rework --->", response)
    ...
    continueTask(id_rework){

      console.log("continuando cronômetro -->")
      
      this.axios.get(`/rework/${id_rework}`)
      .then((response) => {
    ...
    finishTask(){
      ...
      this.axios.post('/rework/finish', json)
      .then((response) => {
        ...
        if(res.status == true ){
            ...
            this.taskDetails.status_task = 4;
            this.taskDetails.real_time_task =  this.day+ ' '+this.hour+':'+this.minute+':'+this.second  ;
    ...
    sendStopTask(){
      ...
      console.log("json stop rework -->", json)

      this.axios.post('/rework/stop', json)
      .then((response) => {
        console.log("response stop rework  --->", response)
        let res =  response.data;
        if(res.status == true ){
            this.Msg(
              "Tarefa pausada.",
    ...
    listRework(){
      ...
      this.axios.get(`/rework/operator/${id}`)
      .then((response) => {
        console.log("response operator rework --->", response)
        let res =  response.data;
        if(res.status == true ){
